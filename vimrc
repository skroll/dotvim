" vim: fdm=marker ts=2 sts=2 sw=2 fdl=2
...
" base configuration {{{
  ...
  set scrolloff=5                                      "threshold before scrolling
  set sidescroll=1
...
" plugin/mapping configuration {{{
  ...
  if count(s:settings.plugin_groups, 'unite') "{{{
    ...
    NeoBundleLazy 'tsukkee/unite-tag', {'autoload':{'unite_sources':['tag','tag/file']}} "{{{
      nnoremap <silent> [unite]t :<C-u>Unite -auto-resize -buffer-name=tag tag<cr>
    "}}}
  ...
  if count(s:settings.plugin_groups, 'misc') "{{{
    ...
    NeoBundleLazy 'Shougo/vimshell.vim', {'autoload':{'commands':[ 'VimShell', 'VimShellInteractive' ]}} "{{{
      ...
      let g:vimshell_temporary_directory=s:get_cache_dir('vimshell')
